...
const CourseCard = memo(({ course }) => {
  ...
  const handleLinkClick = () => {
    scrollTo(0, 0);
    addToViewHistory(course._id, false);
  };

  return (
    <div className="relative bg-white border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-300 rounded-lg overflow-hidden h-full flex flex-col group">
      <button
      ...
      <Link to={'/course/' + course._id} onClick={handleLinkClick} className="flex flex-col h-full">
        ...
        <div className="p-4 text-left flex flex-col flex-1">
          <h3 className="text-lg font-semibold text-gray-800 mb-2 line-clamp-2 min-h-[3.5rem] group-hover:text-blue-600 transition-colors">{course.courseTitle}</h3>
          <p className="text-sm text-gray-600 mb-2">{course.educator?.name || ''}</p>
          ...
          <div className='flex items-center gap-2'>
            ...
            {course.discount > 0 && <p className="text-sm text-gray-400 line-through font-normal">{currency}{course.coursePrice}</p>}
          </div>
        </div>
      </Link>
    </div>
